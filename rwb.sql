...
create table rwb_invite_temp (invite_token varchar(40) not null primary key, invite_name varchar(40) not null, email  varchar(256) not null constraint email_check CHECK (email LIKE '%@%'), referer varchar(64) not null references rwb_users(name) ON DELETE cascade,constraint invite_referer check (referer='root' or referer<>invite_name));
...
CREATE TABLE rwb_permissions_temp (
--
-- must be a current user on the system.  if a user is deleted
-- his permissions should be deleted with him
--
  invite_token  VARCHAR(40) NOT NULL references rwb_invite_temp(invite_token)
     ON DELETE cascade,
--
-- must be a current action on the system.  if an action is deleted
-- then all permissions with that action must also be deleted
--
  invite_action VARCHAR(64) NOT NULL references rwb_actions(action)
     ON DELETE cascade,
--
-- name->action mappings must be unique
--
--
  constraint perm_temp_unique UNIQUE(invite_token,invite_action)
);
--
...
--
INSERT INTO rwb_users (name,password,email,referer) VALUES ('root','abuImgyo/fs5c','root@root.com','root');
INSERT INTO rwb_users (name,password,email,referer) VALUES ('anon','abkHPvHAsaKrQ','anon@anon.com','root');
